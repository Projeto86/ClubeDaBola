...
var twJscSistemaForm = {
	...
	validar : function(vfFormulario){
		for(vf_a = 0; vf_a < vfFormulario.length; vf_a ++){
			var vfCampo = vfFormulario[vf_a];
			var vfValorMinimo = 1;
			if(vfCampo.getAttribute('minimo')!=null)
				vfValorMinimo = parseFloat(vfCampo.getAttribute('minimo')); 
					...
					switch (vfCampo.type){
						case "text":
							if(((vfCampo.getAttribute('opcional')!=null)&&(vfCampo.value.length > 0))||(vfCampo.getAttribute('obrigatorio')!=null)){
								if(twJscSistemaForm.validacao.length.menorQue(vfCampo, vfValorMinimo, vfCampo.getAttribute('mensagem'))){ return(false); }
								if((vfCampo.getAttribute('tipoCpf')!=null)&&(vfCampo.getAttribute('tipoCnpj')!=null)){
									if((!twJscSistemaForm.validacao.tipo.isCpf(vfCampo.value))&&(!twJscSistemaForm.validacao.tipo.isCnpj(vfCampo.value))){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
								} else {
									if(vfCampo.getAttribute('tipoCpf')!=null)
										if(!twJscSistemaForm.validacao.tipo.isCpf(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
									if(vfCampo.getAttribute('tipoCnpj')!=null)
										if(!twJscSistemaForm.validacao.tipo.isCnpj(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
								}
								if(vfCampo.getAttribute('tipoData')!=null)
									if(!twJscSistemaForm.validacao.tipo.isDate(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
								if(vfCampo.getAttribute('tipoEmail')!=null)
									if(!twJscSistemaForm.validacao.tipo.isEmail(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
								if(vfCampo.getAttribute('tipoNumero')!=null)
									if(!twJscSistemaForm.validacao.tipo.isNumeric(vfCampo, vfCampo.getAttribute('mensagem'))){ return(false); }
							}
							break;
						case "password":
							if(((vfCampo.getAttribute('opcional')!=null)&&(vfCampo.value.length > 0))||(vfCampo.getAttribute('obrigatorio')!=null)){
								...
								if(vfCampo.getAttribute('compararCom')!=null){
									for(vf_b = 0; vf_b < vfFormulario.length; vf_b ++){
										var vfCampoComparar = vfFormulario[vf_b];
										if(vfCampoComparar.getAttribute('name') == vfCampo.getAttribute('compararCom')){
						...
						case "file":
							if(((vfCampo.getAttribute('opcional')!=null)&&(vfCampo.value.length > 0))||(vfCampo.getAttribute('obrigatorio')!=null)){
								if(twJscSistemaForm.validacao.length.menorQue(vfCampo, vfValorMinimo, vfCampo.getAttribute('mensagem'))){ return(false); }
								if(vfCampo.getAttribute('tipoImagem')!=null)
									if(!twJscSistemaForm.validacao.tipo.isImage(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
							}
							break;
						case "textarea":
							if(((vfCampo.getAttribute('opcional')!=null)&&(vfCampo.value.length > 0))||(vfCampo.getAttribute('obrigatorio')!=null)){
								if(twJscSistemaForm.validacao.length.menorQue(vfCampo, vfValorMinimo, vfCampo.getAttribute('mensagem'))){ return(false); }
								if(vfCampo.getAttribute('maxlength')!=null)
									if(twJscSistemaForm.validacao.length.maiorQue(vfCampo, parseFloat(vfCampo.getAttribute('maxlength')), vfCampo.getAttribute('mensagem'))){ return(false); }
							}
							break;
						case "checkbox":
						...
						case "radio":
							...
							for(vf_b = 0; vf_b < vfFormulario.length; vf_b ++){
								if(vfFormulario[vf_b].name == vfCampo.name){
									if(vfFormulario[vf_b].checked){ vfAlgumSelecionado = true; break; }
								}
							}
							if(!vfAlgumSelecionado){ alert(vfCampo.getAttribute('mensagem')); vfCampo.focus(); return(false); }
							break;
						case "select-one":
							if(twJscSistemaForm.validacao.length.menorQue(vfCampo, vfValorMinimo, vfCampo.getAttribute('mensagem'))){ return(false); }
							break;
						case "hidden":
							if(((vfCampo.getAttribute('opcional')!=null)&&(vfCampo.value.length > 0))||(vfCampo.getAttribute('obrigatorio')!=null)){
								if(twJscSistemaForm.validacao.length.menorQue(vfCampo, vfValorMinimo, vfCampo.getAttribute('mensagem'), false)){ return(false); }
								if(vfCampo.getAttribute('tipoCpf')!=null)
									if(!twJscSistemaForm.validacao.tipo.isCpf(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); return(false); }
								if(vfCampo.getAttribute('tipoData')!=null)
									if(!twJscSistemaForm.validacao.tipo.isDate(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); return(false); }
								if(vfCampo.getAttribute('tipoEmail')!=null)
									if(!twJscSistemaForm.validacao.tipo.isEmail(vfCampo.value)){ alert(vfCampo.getAttribute('mensagem')); return(false); }
								if(vfCampo.getAttribute('selecoesNumero')!=null){
									if(parseFloat(vfCampo.getAttribute('selecoesNumero'))==0){ 
										alert(vfCampo.getAttribute('mensagem')); 
	...
	input : {
		soNumero : function(snEvento, snWindow){
			var snCaracteresValidos = '0123456789';	
			if(typeof snWindow == 'undefined'){ snWindow = window; }
			var snCodigoCaracter = (snWindow.event) ? snWindow.event.keyCode : snEvento.charCode;
			if ((snCodigoCaracter == 13)||(snCodigoCaracter==0)){ return true; }
			var snCaracter = String.fromCharCode(snCodigoCaracter);
			if (snCaracteresValidos.indexOf(snCaracter) == -1) 
				return false;
			return true;
		},
		soMoeda : function(smEvento, snWindow){
			var smCaracteresValidos = '0123456789.,-';
			if(typeof snWindow == 'undefined'){ snWindow = window; }
			var smCodigoCaracter = (snWindow.event) ? snWindow.event.keyCode : smEvento.charCode;
			if ((smCodigoCaracter == 13)||(smCodigoCaracter==0)){ return true; }
			var smCaracter = String.fromCharCode(smCodigoCaracter);
			if (smCaracteresValidos.indexOf(smCaracter) == -1){ return false; }
			else { return(true); }
		},
		semEnter : function(snEvento, funcaoCallback, snWindow){
			if(typeof snWindow == 'undefined'){ snWindow = window; }
			var snCodigoCaracter = (snWindow.event) ? snWindow.event.keyCode : snEvento.charCode;
			if (snCodigoCaracter == 13){ if(typeof(funcaoCallback)=='function'){ funcaoCallback(); } return false; } else { return true; }
		},
		pula : function(pcCampo){
			if(pcCampo.value.length == pcCampo.maxLength){
				var cname = pcCampo.name!=''?pcCampo.name:pcCampo.id;
				if(cname==''){ return; }
				for(var i=0;i<pcCampo.form.length; i++){
					var fname = pcCampo.form[i].name!=''?pcCampo.form[i].name:pcCampo.form[i].id;
					if(fname == cname){
						if((i+1)<pcCampo.form.length){
							while((pcCampo.form[(i+1)].type == "hidden")&&((i+1)<pcCampo.form.length)){ i++; }
							if((i+1)>=pcCampo.form.length){ break; }
							pcCampo.form[(i+1)].focus();
						}
						break;
					}
				}
